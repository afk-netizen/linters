...
def create_app(test_config='config.py'):
    ...
    def clients():
        if request.method == 'GET':
            clients_list = [client.to_json() for client in db.session.query(Client).all()]
    ...
    def parkings():
        if request.method == 'GET':
            parkings_list = [parking.to_json() for parking in db.session.query(Parking).all()]
            ...
            parking = Parking(address=address, opened=opened, count_places=count_places, count_available_places=count_available_places)
    ...
    def client_parking():
        ...
        if request.method == 'POST':
            ...
            client_parking = db.session.query(ClientParking).where(ClientParking.client_id == client_id,
                                                                   ClientParking.parking_id == parking_id).first()
            if client_parking:
            ...
            client_parkings_list = [client_parking.to_json() for client_parking in db.session.query(ClientParking).all()]
